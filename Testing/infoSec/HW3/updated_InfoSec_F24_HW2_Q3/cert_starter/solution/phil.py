            ...
            if "subjectAltName" in str(ext.get_short_name()):
                ...
                for san_entry in san_list:
                    if san_entry.startswith("DNS:"):
                        ...
                        if cert_domain.count(".") == target_domain.count(".") and fnmatch.fnmatch(target_domain,
                                                                                                  san_domain):
                            check_SAN = True
                            break
...
def check_root(root, trusted_certs):
    ...
    for trusted_cert in trusted_certs:
        if trusted_cert.get_subject().CN:
            if trusted_cert.get_subject().CN.strip() == 'GTS Root R1':
                1
            if trusted_cert.get_subject().CN.strip() == root.get_subject().CN.strip():
                return True
    return False
...
def x509_cert_chain_check(target_domain: str) -> bool:
    ...
    # print(website_root_cert.get_subject().CN)

    # if not check_root(website_root_cert, trusted_certificates):
    #     print("cailed check root")
    #     return False
